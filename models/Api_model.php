...
// 	public function validateCoupon($Coupon = "", $CourseID = ""){
// 	    $this->db->where('code', $Coupon);
// 	    $this->db->where('status', 1);
// 	    $this->db->where('expiry_date >=', date("Y-m-d"));
// 	    $CouponRow = $this->db->get('coupons')->num_rows();
...
//     	    $this->db->where('status', 1);
//     	    $this->db->where('expiry_date >=', date("Y-m-d"));
//     	    $CouponData = $this->db->get('coupons')->result_array();
    	    
//     	    if($CouponData[0]["course_id"] != "0"){
//     	        if($CouponData[0]["course_id"] != $CourseID){
//     	            return array("status" => 0, "message" => "Coupon is not valid for this course");
//     	        }
//     	    }
...
//     	    $Price = ($Course[0]["discounted_price"] > 0) ? $Course[0]["discounted_price"] : $Course[0]["price"];
    	    
//     	    if($CouponData[0]["discount_percentage"] == 2){
//     	        $DiscountedPrice = ($Price * $CouponData[0]["amount"]) / 100;
//     	        $DiscountedPrice = round($DiscountedPrice, 2);
//     	        $GrandTotal = $Price - $DiscountedPrice;
//     	    }else{
//     	        $DiscountedPrice = $CouponData[0]["amount"];
//     	        $DiscountedPrice = round($DiscountedPrice, 2);
//     	        $GrandTotal = $Price - $DiscountedPrice;
//     	    }
    	    
//     	    return array("status" => 1, "discount" => round($DiscountedPrice, 0), "GrandTotal" => round($GrandTotal, 0));
// 	    }else{
// 	        return array("status" => 0, "message" => "Invalid Coupon Code or Code Expired");
// 	    }
// 	}
	public function validateCoupon($Coupon = "", $CourseID = "")
	{
	    if(empty($CourseID) || $CourseID<=0)
            return ["status"=>0,"message"=>"Provide Valid Course Id"];
	    
	    $this->db->where('id', $CourseID);
	    $CourseRow = $this->db->get('course')->row_array();
	    
	    if(empty($CourseRow))
            return ["status"=>0,"message"=>"Invalid Course"];
        
	    if(empty($Coupon))
	    {
	        $Price = ($CourseRow["discounted_price"] > 0) ? $CourseRow["discounted_price"] : $CourseRow["price"];
	        return array("status" => 1, "discount" => 0,"GrandTotal" => round($Price, 0));
        }
	    
	    $this->db->where('code', $Coupon);
	    $this->db->where('status', 1);
	    $CouponRow = $this->db->get('coupons')->row_array();
	    
	    if(empty($CouponRow))
            return ["status"=>0,"message"=>"Invalid Coupon Code"];
	    
	    if($CourseID!=$CouponRow['course_id'] && $CouponRow['course_id']!=0)
	        return ["status"=>0,"message"=>"This Coupon Code Is Not Valid For This Course"];
	        
	    if(date("Y-m-d") > $CouponRow['expiry_date'])
	        return ["status"=>0,"message"=>"This Coupon Code Is Expired"];
	        
	    $Price = ($CourseRow["discounted_price"] > 0) ? $CourseRow["discounted_price"] : $CourseRow["price"];
    	
    	$DiscountedPrice    =   $CouponRow["amount"];
        $DiscountedPrice    =   round($DiscountedPrice, 2);
        $GrandTotal         =   $Price - $DiscountedPrice;
	        
	    if($CouponRow["discount_percentage"] == 2)
	    {
	        $DiscountedPrice    =   ($Price * $CouponRow["amount"]) / 100;
	        $DiscountedPrice    =   round($DiscountedPrice, 2);
	        $GrandTotal         =   $Price - $DiscountedPrice;
	    }
	...
	public function CourseDetail($ID = "", $UserID = ""){
		if ($ID != "") {
			$this->db->where('id', $ID);
		}
		...
		$Rating = $this->db->get_where('rating', array('ratable_type' => 'course', 'ratable_id' => $ID, "user_id" => $UserID))->row();
		
		$result[0]['rate_by_me'] = $Rating->rating == "" ? 0 : $Rating->rating;
	...
	public function new_courses_get($top_course_id = "")
	{
		if ($top_course_id != "") {
			$this->db->where('id', $top_course_id);
		}
		$this->db->where('is_new_course', 1);
		$this->db->where('status', 'active');
		$this->db->order_by("id", "DESC");
		$top_courses = $this->db->get('course')->result_array();
	...
	public function self_courses_get($top_course_id = "")
	{
		if ($top_course_id != "") {
			$this->db->where('id', $top_course_id);
		}
		$this->db->where('is_self_course', 1);
		$this->db->where('status', 'active');
		$this->db->order_by("id", "DESC");
		$top_courses = $this->db->get('course')->result_array();
	...
	public function categories_get($category_id)
	{
		...
		foreach ($categories as $key => $category) {
			...
			$categories[$key]['number_of_sub_categories'] =  $this->db->get_where('category',array('parent'=>$category['id']))->num_rows();
		}
		return $categories;
	}
	public function all_categories_get($category_id)
	{
		if ($category_id != "") {
			$this->db->where('id', $category_id);
		}
		...
		foreach ($categories as $key => $category) {
			$categories[$key]['thumbnail'] = $this->get_image('category_thumbnail', $category['thumbnail']);
			$categories[$key]['number_of_courses'] = $this->crud_model->get_category_wise_courses($category['id'])->num_rows();
		$categories[$key]['sub_categories'] =  $this->db->get_where('category',array('parent'=>$category['id']))->result_array();
		}
		return $categories;
	}

	public function sub_categories_get($category_id)
	{
	...
	{
	    $selected_category = $_GET['selected_category'] ? $_GET['selected_category'] : 'all';
		$selected_price = $_GET['selected_price'] ? $_GET['selected_price'] : 'all';
		$selected_level = $_GET['selected_level'] ? $_GET['selected_level'] : 'all';
		$selected_language = $_GET['selected_language'] ? $_GET['selected_language'] : 'all';
		$selected_rating = $_GET['selected_rating'] ? $_GET['selected_rating'] : 'all';
...
// 		}
	...
	// Get category_type
	public function category_type_get($category_type_id)
	{
	    if ($category_type_id != "") {
            $this->db->where('id', $category_type_id);
        }
        
        $category_type = $this->db->get('category_type')->result_array();
        
        if ($category_type_id == "") {
            $category_type[] = ['id'=>'live_class','type'=>'live class'];
        }
        return $category_type;
	}
	
	public function course_type_get($category_type_id)
	{
	   
            $this->db->where('category_type', $category_type_id);
        
        
        $category_type = $this->db->get('course')->result_array();
	...
	public function course_data($courses = array())
	{
		foreach ($courses as $key => $course) {
		    ...
		    $courses[$key]['description'] = strip_tags(trim($course['description']));
		  //  $courses[$key]['videos'] = strip_tags(trim($course['videos']));
...
// 			$courses[$key]['thumbnail'] = $this->get_image('course_thumbnail', $course['id']);
			$courses[$key]['thumbnail'] = $course['thumbnail'];
			if ($course['is_free_course'] == 1) {
				$courses[$key]['price'] = get_phrase('free');
				$courses[$key]['unitPrice'] = 0;
			} else {
				if ($course['discount_flag'] == 1) {
					$courses[$key]['price'] = currency($course['price']);
					$courses[$key]['unitPrice'] = $course['discounted_price'];
				} else {
					$courses[$key]['price'] = currency($course['price']);
					$courses[$key]['unitPrice'] = $course['price'];
				}
			}
			...
			$courses[$key]['instructor_id'] = $course['user_id'];
	...
	public function login_get()
	{
		...
		if ($query->num_rows() > 0) {
			...
			$userdata['profile'] = $row['image'];
			$userdata['image'] = base_url('uploads/user_image/'.$row['image']);
			$userdata['role'] = strtolower(get_user_role('user_role', $row['id']));
			$userdata['validity'] = 1;
			
			$this->db->where('id', $row['id']);
			$this->db->update('users', array('fcm_token' => $_POST['token']));
        ...
        if($validity === 'unverified_user' || $validity == true) {
        	if($validity === true){
		        ...
		        if($this->db->insert_id() > 0){
		          ...
		          $this->email_model->register($emaildata['email'], $emaildata['first_name'], $emaildata['last_name']);
            ...
            if (get_settings('student_email_verification') == 'enable') {
                ...
                $this->email_model->register($data['email'], $data['first_name'], $data['last_name']);
            	$response['message'] = get_phrase('registration_successful');
            	$response['email_verification'] = get_settings('student_email_verification');
            }
        } else {
	...
	public function verify_email_address_post(){
  	...
  	public function update_fcm_token_post(){
        $response = array();
        $credentials = array('fcm_token' => $_POST['token']);
        $this->db->where('id', $_POST['id']);
        $this->db->update('users', $credentials);
	...
	public function resend_verification_code_post(){
  	...
  	public function forgot_password(){
		...
		if (!empty($_POST['email'])) {
		    ...
		    if($query->num_rows() > 0) {
		        $new_password = substr(str_shuffle("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"), 10, 10);
		        $data['password'] = sha1($new_password);
				$this->db->where('id', $query->row('id'));
				$this->db->update('users', $data);
				
		        $this->email_model->password_reset_email($new_password, $query->row('email'));
		        $response['message'] = "New Password Send On Your Email. Please Check";
    	      	$response['status'] = 200;
    	      	$response['validity'] = true;
		    }else{
	...
	public function my_courses_get($user_id = "")
	{
		...
		if ($selected_price != "all")
			$this->db->where('is_free_course', ($selected_price == "paid") ? '0' : '1');
		
		
		$this->db->select('id');
		$courses = $this->db->get('course')->result_array();
		
		$course_ids = (!empty($courses)) ? array_column($courses,'id') : [];
		
		if($selected_rating!='all')
		{
		    $this->db->select('ratable_id');
		    $this->db->where('ratable_type','course');
		    $this->db->where('user_id',$user_id);
		    $this->db->where('rating',$selected_rating);
		    $ratingData = $this->db->get('rating')->result();
		    
		    $course_ids = (!empty($ratingData)) ? array_intersect($course_ids,array_column($ratingData,'ratable_id')) : $course_ids;
		}
		...
		$my_courses_ids = $this->db->get('enrol')->result();
// 		$this->db->select('course_id')->where('user_id',$user_id)->get('enrol')->result();
		if(!empty($my_courses_ids))
		{
		    $my_courses_ids = array_column($my_courses_ids,'course_id');
		    
		    $coursedata = $this->db->select('*')->where_in('id', $my_courses_ids)->get('course')->result_array();
		    
		    $my_courses = $this->course_data(array_filter($coursedata));
		...
		$my_courses = $this->course_data(array_filter($my_courses));
		foreach ($my_courses as $key => $my_course) {
			if (isset($my_course['id']) && $my_course['id'] > 0) {
				$my_courses[$key]['completion'] = round(course_progress($my_course['id'], $user_id));
				$my_courses[$key]['total_number_of_lessons'] = $this->crud_model->get_lessons('course', $my_course['id'])->num_rows();
	...
	{
	    $course_details = $this->db->get_where('course', array('id' => $course_id))->result_array();
...
// 		$response[0]['price'] = $course_details[0]['price'];
// 		$response[0]['discount_flag'] = $course_details[0]['discount_flag'];
// 		$response[0]['discounted_price'] = $course_details[0]['discounted_price'];
		...
		foreach ($response as $key => $resp) {
			...
			$followdata = $this->db->where('user_id',$user_id)->where('instructor_id',$resp['user_id'])->get('tbl_follow')->row();
			
			$response[$key]['follow_status'] = (!empty($followdata)) ? true : false;
			...
			#new added line for rating/review list start
			$response[$key]['course_review'] = [];
		    $RatingData = $this->db->query("SELECT CONCAT(users.first_name,' ',users.last_name) as user_name,users.image as user_profile,rating.id as rating_id,review as comment,rating.rating,rating.date_added as entry_date FROM `rating` INNER JOIN users ON users.id=rating.user_id WHERE ratable_type='course' AND ratable_id='$course_id'")->result_array();
    	    if(!empty($RatingData))
    	    {
	           foreach($RatingData as $rkey => $Rvalue)
	           {
	               $RatingData[$rkey]['user_profile'] = base_url('uploads/user_image/'.((!empty($Rvalue['user_profile']) && file_exists('uploads/user_image'.$Rvalue['user_profile'])) ? $Rvalue['user_profile'] : 'no_image.png'));
	                $RatingData[$rkey]['entry_date'] = date('d,M Y',$Rvalue['entry_date']);
	           }
	          $response[$key]['course_review'] = $RatingData;
    	    }
            #new added line for rating/review list close
		}
		return $response;
	}
	...
	public function update_password_post($user_id = 0)
	{
		$response = array();
		...
		if(empty($_POST['current_password']))
		    return ['status'=>'failed','message'=>'Current password is required'];
		    
		if(empty($_POST['new_password']))
		    return ['status'=>'failed','message'=>'New password is required'];
		    
		if(empty($_POST['confirm_password']))
		    return ['status'=>'failed','message'=>'Confirm password is required'];
		
		if($_POST['current_password'] == $_POST['new_password'])
		    return ['status'=>'failed','message'=>'Current password and new password are same'];
		    
		if($_POST['confirm_password'] != $_POST['new_password'])
		    return ['status'=>'failed','message'=>'Confirm password does not match with password'];
		...
		$this->db->where('id',$user_id)->update('users',['password'=>sha1($_POST['new_password'])]);
		return ['status'=>'success','message'=>'Password update successfully'];
...
// 		return $response;
	...
	public function save_request_support()
	{
	    
	      
	    
		$this->db->insert('support_contact', $_POST);
	}
