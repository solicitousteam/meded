...
class Crud_model extends CI_Model
{
    ...
    public function saveNotification($Title, $Desp)
    {
        ...
        foreach ($Students as $STD) {
            ...
            if ($STD->fcm_token != "") {
                ...
                $NotificationData["content"] = $Notification;
                $arrayToSend = array('to' => $UserToken, "notification" => $notification, "data" => $NotificationData);
                $json = json_encode($arrayToSend);

                $headers = array();
                $headers[] = 'Content-Type: application/json';
                $headers[] = 'Authorization: key=AAAA14qKAws:APA91bGE_cUR-V3OgkExtXr-S7_SZKbBN4F69fnVq4QZYLfTgGqblegT_IFOCN9UUiFYDCRSus1blEItLusiAPPNwU_46_ktudweZns1nMxZR-CLZOD-YvHXlAsJDoC4twPs9oVKTcNE';
    ...
    {
        return $this->db->order_by('name','ASC')->get_where('category', array('parent' => $parent_id))->result_array();
    }
    ...
    public function enrol_history_by_date_range($timestamp_start = "", $timestamp_end = "", $user_id = 0)
    {
        if ($user_id > 0) {
        ...
        if (isset($_GET['date'])) {
            $Conditions['date_added >='] = strtotime($_GET['date']);
        } else {
        ...
        if ($user_id > 0)
            $this->db->where_in('course_id', array_column($CourseData, 'course_id'));

        return $this->db->get('enrol');
    }


    public function enrol_history_by_date_range2($date)
    {
        $this->db->where('date_added >=', strtotime($date));
        return $this->db->get('enrol');
    }

    public function get_revenue_by_user_type($pagedata, $revenue_type = "")
    {
        $course_ids = array();
        $courses    = array();
        ...
        $timestamp_start =   $pagedata['timestamp_start'];
        $timestamp_end =   $pagedata['timestamp_end'];
        ...
        if ($revenue_type == 'admin_revenue') {
            ...
            if (!empty($pagedata['timestamp_start'])) {
                $this->db->where('date_added >=', $timestamp_start);
                $this->db->where('date_added <=', $timestamp_end);
            }
    ...
    public function get_followers($type = "", $id = "")
    {
        $this->db->order_by("users.id", "asc");
        if ($type == "course") {
            $this->db->select("users.fcm_token");
            $this->db->join("course", "tbl_follow.instructor_id = course.user_id", 'left');
            $this->db->join("users", "tbl_follow.user_id = users.id", 'left');
            $this->db->where("course.id", $id);
            return $this->db->get('tbl_follow');
        } elseif ($type == "serial") {
            $this->db->select("users.fcm_token");
            $this->db->join("serial_learning", "tbl_follow.instructor_id = serial_learning.user_id", 'left');
            $this->db->join("users", "tbl_follow.user_id = users.id", 'left');
            $this->db->where("serial_learning.id", $id);
            return $this->db->get('tbl_follow');
        } elseif ($type == "video") {

            $this->db->select("users.fcm_token");
            $this->db->join("video_learning", "tbl_follow.instructor_id = video_learning.user_id", 'left');
            $this->db->join("users", "tbl_follow.user_id = users.id", 'left');
            $this->db->where("video_learning.id", $id);
            return $this->db->get('tbl_follow');
        } elseif ($type == "webinar") {

            $this->db->select("users.fcm_token");
            $this->db->join("webinar", "tbl_follow.instructor_id = webinar.created_by", 'left');
            $this->db->join("users", "tbl_follow.user_id = users.id", 'left');
            $this->db->where("webinar.id", $id);
            return $this->db->get('tbl_follow');
        } else {
            return $this->db->get('users.fcm_token');
        }
    }


    public function add_serial($param1 = "")
    {

        $data['title'] = html_escape($this->input->post('title'));
        $data['topic'] = html_escape($this->input->post('topic'));
        $data['category_id'] = $category_details['category_id'];
        ...
        if ($_FILES["course_thumbnail"]['name'] != "") {
            ...
            move_uploaded_file($_FILES["course_thubmnail"]['tmp_name'], 'uploads/thumbnails/course_thumbnails/' . '_' . get_frontend_settings('theme') . '_' . $course_id . '.jpg');
        }

        if ($data['status'] == 'approved') {
            $this->session->set_flashdata('flash_message', get_phrase('serial_learning_added_successfully'));
        } elseif ($data['status'] == 'pending') {
            $this->session->set_flashdata('flash_message', get_phrase('serial_learning_added_successfully') . '. ' . get_phrase('please_wait_untill_Admin_approves_it'));
        } elseif ($data['status'] == 'draft') {
            $this->session->set_flashdata('flash_message', get_phrase('your_serial_learning_has_been_added_to_draft'));
        }
    ...
    //add video learning
    public function add_video_learning($param1 = "")
    {
        $outcomes = $this->trim_and_return_json($this->input->post('outcomes'));
        $objectives = $this->trim_and_return_json($this->input->post('objectives'));
        $data['title'] = html_escape($this->input->post('title'));
        $data['description'] = html_escape($this->input->post('description'));
        ...
        $data['is_top_course'] = (!empty($this->input->post('is_top_course'))) ? 1 : 0;
        $data['is_new_course'] = (!empty($this->input->post('is_new_course'))) ? 1 : 0;
        $data['is_self_course'] = (!empty($this->input->post('is_self_course'))) ? 1 : 0;
        $data['is_featured_course'] = (!empty($this->input->post('is_featured_course'))) ? 1 : 0;
        ...
        #---------------------------------------------------------------------------------
        #thumbnail section
        if (!file_exists('uploads/thumbnails/video_thumbnails'))
            mkdir('uploads/thumbnails/video_thumbnails', 0777, true);
        
        if($_FILES['course_thumbnail']['name'])
            $data['thumbnail'] = $this->upload_file('uploads/thumbnails/video_thumbnails/', 'course_thumbnail',0);
        #---------------------------------------------------------------------------------

        #---------------------------------------------------------------------------------
        #thumbnail section
        if (!file_exists('uploads/video/video_learning'))
            mkdir('uploads/video/video_learning', 0777, true);
            
        if($_FILES['course_video']['name'])
                $data['video_url'] = $this->upload_file('uploads/video/video_learning/', 'course_video',0);
        #---------------------------------------------------------------------------------
        
        $data['proposed_date'] = (!empty($_POST['proposed_date'])) ? date('Y-m-d',strtotime($_POST['proposed_date'])) : '';
        ...
        //         // Upload different number of images according to activated theme. Data is taking from the config.json file
        //         $course_media_files = themeConfiguration(get_frontend_settings('theme'), 'course_thubmnail');
        // //        foreach ($course_media_files as $course_media => $size) {
        //             if ($_FILES["course_thubmnail"]['name'] != "") {
        //                 $course_media='course_thumbnail';
        //                   move_uploaded_file($_FILES["course_thubmnail"]['tmp_name'], 'uploads/thumbnails/video_thumbnails/' . $course_media . '_' . get_frontend_settings('theme') . '_'. $course_id . '.jpg');
        //          }
        //   //      }
        ...
        if ($_FILES["course_video"]['name'] != "")
        {
            ...
            $upload_path = 'uploads/video/video_learning';
...
//            $config['file_name'] = $course_id;
            ...
            if ($is_file_error) {
                ...
                $data['video_url'] = base_url().'uploads/video/video_learning/'.$video_data['file_name'];
                $this->db->update('course',$data,'id='.$course_id);
    ...
    // edit video learning
    public function edit_video_learning($course_id = "")
    {
        // echo '<pre>';
        // print_r($_POST);
        ...
        $this->db->update('video_learning', $data);
        ...
        $course_old_details = $this->get_course_by_id($course_id)->row_array();
        ...
        if($_FILES['course_thumbnail']['name'])
        {
            $data['thumbnail'] = $this->upload_file('uploads/thumbnails/video_thumbnails/', 'course_thumbnail',$course_id);
            
            if(!empty($course_old_details['thumbnail']) && file_exists(str_replace(base_url(),'',$course_old_details['thumbnail'])))
                unlink(str_replace(base_url(),'',$course_old_details['thumbnail']));
        }
        #---------------------------------------------------------------------------------
        ...
        if($_FILES['course_video']['name'])
        {
                $data['video_url'] = $this->upload_file('uploads/video/video_learning/', 'course_video',$course_id);
                
                if(!empty($course_old_details['video_url']) && file_exists(str_replace(base_url(),'',$course_old_details['video_url'])))
                    unlink(str_replace(base_url(),'',$course_old_details['video_url']));
        }
        #---------------------------------------------------------------------------------
    ...
    //add serial learning
    public function add_serial_learning($param1 = "")

    {
        // echo '<prE>';
        // print_r($_POST);
        // // print_r(count($_FILES['course_video']));die;
        // print_r($_FILES);
        $requirement = '-';
        $outcomes = $this->trim_and_return_json($this->input->post('outcomes'));
        $objectives = $this->trim_and_return_json($this->input->post('objectives'));
        ...
        #-----------------------------------------------------------------------
        #thumbnail section
        if (!file_exists('uploads/thumbnails/serial_learning_thumbnails'))
            mkdir('uploads/thumbnails/serial_learning_thumbnails', 0777, true);
        
        if($_FILES['course_thumbnail']['name'])
            $data['thumbnail'] = $this->upload_file('uploads/thumbnails/serial_learning_thumbnails/', 'course_thumbnail',0);
        #-----------------------------------------------------------------------
        ...
        if ($_FILES["course_thubmnail"]['name'] != "") {
            ...
            move_uploaded_file($_FILES["course_thubmnail"]['tmp_name'], 'uploads/thumbnails/course_thumbnails/' . $course_media . '_' . get_frontend_settings('theme') . '_' . $course_id . '.jpg');
        }
        //      }
        */
        ...
        // Create folder if does not exist
        if (!file_exists('uploads/video/serial_learning'))
            mkdir('uploads/video/serial_learning', 0777, true);
            
        if (isset($_FILES["course_video"]) && is_array($_FILES["course_video"]['name']))
        {
            //set preferences
            //file upload destination
            $upload_path = 'uploads/video/serial_learning';
            $config['upload_path'] = $upload_path;
            $config['allowed_types'] = 'wmv|mp4|avi|mov';
            //allowed max file size. 0 means unlimited file size
            $config['max_size'] = '0';
            ...
            $video_names = array();
            $count = count($_FILES["course_video"]['name']);
            ...
            for ($key = 0; $key < $count; $key++) {
                ...
                if(!empty($_FILES['course_video']['type'][$key]))
                {
                    $upload_path = 'uploads/video/serial_learning/video_'.$course_id.'_'.$key.time().'.'.explode('.',$_FILES['course_video']['name'][$key])[1];
                    move_uploaded_file($_FILES['course_video']['tmp_name'][$key], $upload_path);
                    $video_url = base_url($upload_path);
                    
                    $serial_video = array(
                        'serial_id' => $course_id,
                        'video_title' => (!empty($video_title[$key])) ? $video_title[$key] : '',
                        'video_description' => (!empty($video_description[$key])) ? $video_description[$key] : '',
                        'video_url' => $video_url
                    );
                    $this->db->insert('serial_videos', $serial_video);
                    ...
                    if($key == 0)
                    {
                        ...
                        $this->db->where('id',$course_id)->update('course',$UpdateCourseData);
                    }
                }
            }
        }
    ...
    //edit serial learning
    public function edit_serial_learning($param1 = "")
    {
        ...
        if($_FILES['course_thumbnail']['name'])
        {
            $data['thumbnail'] = $this->upload_file('uploads/thumbnails/course_thumbnails/', 'course_thumbnail',$course_id);
            
            if(!empty($course_details['thumbnail']) && file_exists(str_replace(base_url(),'',$course_details['thumbnail'])))
                unlink(str_replace(base_url(),'',$course_details['thumbnail']));
        }
        #-----------------------------------------------------------------------
        // echo '<prE>';print_r($data);die();
        $this->db->where('id', $course_id);
        ...
        if (isset($_FILES["course_video"]) && is_array($_FILES["course_video"]['name']))
        {
            $oldvideosdata = $this->db->select('video_url')->where('serial_id',$course_id)->get(
                'serial_videos')->result();
            if(!empty($oldvideosdata))
            {
                $url_array = array_column($oldvideosdata,'serial_videos');
                foreach($url_array as $val)
                {
                    $url = str_replace(base_url(),'',$val);
            ...
            $this->db->where('serial_id',$course_id)->delete('serial_videos');
        ...
        $this->session->set_flashdata('flash_message', get_phrase('course_has_been_updated_successfully'));
        return $course_id;
    }

    public function userNotifiation($user_id)
    {
    ...
    public function add_course($param1 = "")
    {
        ...
        $outcomes = $this->trim_and_return_json($this->input->post('outcomes'));
        $objective = $this->trim_and_return_json($this->input->post('objectives'));
        $requirements = $this->trim_and_return_json($this->input->post('requirements'));
        ...
        $data['is_free_course'] = (!empty($this->input->post('is_free_course'))) ? 1 : 0;
        ...
        if($_FILES['course_thumbnail']['name'])
            $data['thumbnail'] = $this->upload_file('uploads/thumbnails/course_thumbnails/', 'course_thumbnail',0);
        
        #------------------------- 04-06-2022 ------------------------------

        // Create folder if does not exist
        if (!file_exists('uploads/video/courses')) {
            mkdir('uploads/video/courses', 0777, true);
        }

        $data['course_overview_provider'] = $this->input->post('course_overview_provider');
        $data['video_courtesy'] = $this->input->post('video_courtesy');

        $data['video_url'] = $this->input->post('course_overview_url');
        if (trim(strtolower($this->input->post('course_overview_provider'))) == 'other')
        {
            if (!empty($_FILES['course_overview_url']['tmp_name']))
            {
                $video_url = $_FILES['course_overview_url']['tmp_name'];
                
                if($_FILES['course_overview_url']['name'])
                    $data['video_url'] = $this->upload_file('uploads/video/courses/', 'course_overview_url',0);
            
                // $upload_path = 'uploads/video/courses/blended_course_video_' . rand(1199, 9999) . time() . '.'.explode('.',$_FILES['course_overview_url']['name'])[1];

                // move_uploaded_file($video_url, $upload_path);

                // $data['video_url'] = base_url($upload_path);
            }
        }
        ...
        // if(!empty($_FILES["course_thubmnail"]['name']))
        // {
        //     // $thumbnail = 'uploads/thumbnails/course_thumbnails/serail_course_'.date('dmY').'_'.time().rand(111,9999).'.'.explode('.',$_FILES["course_thubmnail"]['name'])[1];
        //     $thumbnail = 'uploads/thumbnails/course_thumbnails/blended_course_'.date('dmY').'_'.time().$course_id.rand(111,9999).'.'.explode('.',$_FILES["course_thubmnail"]['name'])[1];
        //     move_uploaded_file($_FILES["course_thubmnail"]['tmp_name'], $thumbnail);
        //     $data['thumbnail'] = base_url($thumbnail);
        // }
        
        unset($data['section']);
        unset($data['section_sub_name']);
        unset($data['section_quiz_title']);
        unset($data['section_quiz_instruction']);
        unset($data['question_name']);
        unset($data['correct_answer']);
        unset($data['question_option']);
        unset($data['section_lesson_type']);
        unset($data['section_lesson_title']);
        unset($data['lesson_form']);
        
        if(!empty($data['total_section']))
        {
            for($i=0;$i<=$data['total_section'];$i++)
            {
                unset($data['section_sub_div_'.$i]);
            }
        }
        unset($data['section_div']);
        unset($data['total_section']);
        ...
        if ($course_id) {
            ...
            $message = rawurlencode("[MedEd]: Hello, $fname You have successfully created a $title course. For more info: meded.testing@gmail.com.");
    ...
    public function update_course($course_id, $type = "")
    {
        ...
        if (trim(strtolower($this->input->post('course_overview_provider'))) == 'youtube')
            $data['video_url'] = $this->input->post('course_overview_url');
        
        if (trim(strtolower($this->input->post('course_overview_provider'))) == 'other')
        {
            $data['video_url'] = $course_details['video_url'];
            if ($_FILES['course_overview_url']['size'] > 0)
            {
                if($_FILES['course_overview_url']['name'])
                    $data['video_url'] = $this->upload_file('uploads/video/courses/', 'course_overview_url',$course_id);
            }
        }
        
        #remove old video from folder is exists
        if(!empty($_FILES['course_overview_url']['tmp_name']) && $course_details['course_overview_provider'] == 'other' && !empty($course_details['video_url']) && file_exists(str_replace(base_url(),'',$course_details['video_url'])))
            unlink(str_replace(base_url(),'',$course_details['video_url']));
    ...
    public function change_serial_learning_status($status = "", $course_id = "")
    {
        if ($status == 'active') {
            if ($this->session->userdata('admin_login') != true) {
                redirect(site_url('login'), 'refresh');
    ...
    public function change_video_learning_status($status = "", $course_id = "")
    {
        if ($status == 'active') {
            if ($this->session->userdata('admin_login') != true) {
                redirect(site_url('login'), 'refresh');
    ...
    public function change_webinar_status($status = "", $course_id = "")
    {
        if ($status == 'active') {
            if ($this->session->userdata('admin_login') != true) {
                redirect(site_url('login'), 'refresh');
        ...
        $this->db->where('id', $course_id);
        $this->db->update('webinar', $updater);
    }
    ...
    public function get_course_thumbnail_url($course_id, $type = 'course_thumbnail')
    {
        ...
        // }
      $img = $this->db->where('id', $course_id)->get('course')->row();
    ...
    public function get_video_learning_thumbnail_url($course_id, $type = 'video_learning_thumbnails')
    {
        // Course media placeholder is coming from the theme config file. Which has all the placehoder for different images. Choose like course type.
        $course_media_placeholders = themeConfiguration(get_frontend_settings('theme'), 'course_media_placeholders');
        // if (file_exists('uploads/thumbnails/course_thumbnails/'.$type.'_'.get_frontend_settings('theme').'_'.$course_id.'.jpg')){
        ...
        //     return $course_media_placeholders[$type.'_placeholder'];
        // }
        if (file_exists('uploads/thumbnails/' . $type . '/' . $course_id . '.jpg')) {
            return base_url() . 'uploads/thumbnails/' . $type . '/' . $course_id . '.jpg';
        } else {
            // return base_url() . $course_media_placeholders[$type . '_placeholder'];
        }
    }

    public function get_webinar_thumbnail_url($course_id, $type = 'webinar')
    {
        // Course media placeholder is coming from the theme config file. Which has all the placehoder for different images. Choose like course type.
        $course_media_placeholders = themeConfiguration(get_frontend_settings('theme'), 'course_media_placeholders');
        // if (file_exists('uploads/thumbnails/course_thumbnails/'.$type.'_'.get_frontend_settings('theme').'_'.$course_id.'.jpg')){
        ...
        //     return $course_media_placeholders[$type.'_placeholder'];
        // }
        if (file_exists('uploads/' . $type . '/' . $course_id . '.jpg')) {
            return base_url() . 'uploads/' . $type . '/' . $course_id . '.jpg';
        } else {
            return base_url() . $course_media_placeholders[$type . '_placeholder'];
        }
    }
    ...
    public function get_serial_learning_thumbnail_url($lesson_id)
    {
    ...
    public function get_video_learning_by_id($course_id = "")
    {
        return $this->db->get_where('course', array('id' => $course_id));
    }
    public function get_serial_learning_by_id($course_id = "")
    {
        return $this->db->get_where('course', array('id' => $course_id));
    }
    ...
    public function course_count($par,$par2)
    {
        
        $this->db->select('course.*,users.first_name,users.last_name');
        $this->db->from('course');
        $this->db->join('users', 'users.id=course.user_id');
        $this->db->where('course.status', $par);
        $this->db->where('course.category_type', $par2);

         $courses = $this->db->get('')->result_array();
        //  $courses = $this->db->get_where('course', array('status' => $par,'category_type' => $par2));
          return $courses;
     ...
     public function free_counti()
    {
         $courses = $this->db->get_where('course',array('user_id'=> $this->session->userdata('user_id'),'is_free_course'=> 1,'category_type' => 1,'status' => 'active'));
         return $courses;
        
        
    }
    
      public function paid_counti()
    {
         $courses = $this->db->get_where('course',array('user_id'=> $this->session->userdata('user_id'),'is_free_course' =>  0,'category_type' => 1,'status' => 'active'));
         return $courses;
    ...
    {
         $courses = $this->db->get_where('course',array('is_free_course'=> 1,'category_type' => 1,'status' => 'active'));
         return $courses;
    ...
    {
         $courses = $this->db->get_where('course',array('is_free_course' =>  0,'category_type' => 1,'status' => 'active'));
         return $courses;
      ...
      public function free_count_blended($a,$b)
    {
        //  $courses = $this->db->get_where('course',array('is_free_course'=> $a,'category_type' => $b,'status' => 'active'));
        //  return $courses;
        ...
        $this->db->where('course.status', 'active');
         $this->db->where('course.is_free_course', $a);
        $this->db->where('course.category_type', $b);
    ...
    public function get_status_wise_courses($status = "", $category_type = 0)
    {
        
        $category_column = ((int)$category_type >= 0) ? 'category_type' : 'category_type>=';
        ...
        {
            $user_column = 'user_id';
            $user_value = $_GET['instructor_id'];
        }
        
        if (addon_status('scorm_course')) {
            if ($status != "") {
                $courses = $this->db->get_where('course', array('status' => $status, $category_column => $category_type,$user_column=>$user_value));
            } else {
                $courses['draft'] = $this->db->get_where('course', array('status' => 'draft',$user_column=>$user_value));
                $courses['pending'] = $this->db->get_where('course', array('status' => 'pending',$user_column=>$user_value));
                $courses['active'] = $this->db->get_where('course', array('status' => 'active',$user_column=>$user_value));
            }
        } else {
            if ($status != "") {
                //'course_type' => 'general',
    ...
    public function get_status_wise_courses_for_instructor($status = "", $category_type = '0')
    {
        if ($status != "") {
    ...
    // Course List
    public function get_courses_list($status = '', $category_type = '', $is_free = '')
    {
        ...
        if((int)$category_type >= 0)
            $this->db->where('course.category_type', $category_type);
    ...
    public function update_blended_learning()
    {
        if (isset($_FILES['blended_learning']) && $_FILES['blended_learning']['name'] != "") {
            unlink('uploads/system/' . get_frontend_settings('blended_learning'));
            $data['value'] = md5(rand(1000, 100000)) . '.png';
            $this->db->where('key', 'blended_learning');
            $this->db->update('frontend_settings', $data);
            move_uploaded_file($_FILES['blended_learning']['tmp_name'], 'uploads/system/' . $data['value']);
        }
        //move_uploaded_file($_FILES['favicon']['tmp_name'], 'uploads/system/favicon.png');
    }

    public function update_video_learning()
    {
        if (isset($_FILES['video_learning']) && $_FILES['video_learning']['name'] != "") {
            unlink('uploads/system/' . get_frontend_settings('video_learning'));
            $data['value'] = md5(rand(1000, 100000)) . '.png';
            $this->db->where('key', 'video_learning');
            $this->db->update('frontend_settings', $data);
            move_uploaded_file($_FILES['video_learning']['tmp_name'], 'uploads/system/' . $data['value']);
        }
        //move_uploaded_file($_FILES['favicon']['tmp_name'], 'uploads/system/favicon.png');
    }

    public function update_serial_learning()
    {
        if (isset($_FILES['serial_learning']) && $_FILES['serial_learning']['name'] != "") {
            unlink('uploads/system/' . get_frontend_settings('serial_learning'));
            $data['value'] = md5(rand(1000, 100000)) . '.png';
            $this->db->where('key', 'serial_learning');
            $this->db->update('frontend_settings', $data);
            move_uploaded_file($_FILES['serial_learning']['tmp_name'], 'uploads/system/' . $data['value']);
        }
        //move_uploaded_file($_FILES['favicon']['tmp_name'], 'uploads/system/favicon.png');
    }

    public function update_webinars()
    {
        if (isset($_FILES['webinars']) && $_FILES['webinars']['name'] != "") {
            unlink('uploads/system/' . get_frontend_settings('webinars'));
            $data['value'] = md5(rand(1000, 100000)) . '.png';
            $this->db->where('key', 'webinars');
            $this->db->update('frontend_settings', $data);
            move_uploaded_file($_FILES['webinars']['tmp_name'], 'uploads/system/' . $data['value']);
        }
        //move_uploaded_file($_FILES['favicon']['tmp_name'], 'uploads/system/favicon.png');
    }
    ...
    public function get_enrol_student_count($course)
    {
        $data['course_id'] = $course;
        return $this->db->get_where('enrol', $data)->num_rows();
    }
    ...
    function send_new_private_message()
    {
        ...
        if ($this->session->userdata('user_id') == $message_thread_details['sender']) {
            $receiver = $message_thread_details['receiver'];
        } else {
            $receiver = $message_thread_details['sender'];
        }
    ...
    */
    ...
    function mark_thread_messages_read($message_thread_code)
    {
        ...
        // echo 'hi, you are now in crud_model';die;
    }
    ...
    public function get_last_message_by_message_thread_code($message_thread_code)
    {
        ...
        $this->db->limit(1);
        $this->db->where(array('message_thread_code' => $message_thread_code, 'sender!=' => $this->session->userdata('user_id')));
    ...
    public function get_courses($category_id = 0, $sub_category_id = 0, $instructor_id = 0)
    {
        if ($category_id > 0)
            $this->db->where('category_id', $category_id);
    ...
    public function filter_course_for_backend($category_id, $instructor_id, $price, $status, $search = '')
    {
        ...
        if(!empty($_GET['date']))
        {
            $this->db->where('course.date_added>=',strtotime($_GET['date']));
        }
    ...
    public function filter_serialNumber_for_backend($category_id, $instructor_id, $price, $status)
    {
        if ($category_id != "all") {
    ...
    //video learning
    public function filter_video_learning_for_backend($category_id, $instructor_id, $price, $status, $search = '')
    {
        $this->db->select('course.*,users.first_name,users.last_name');
        $this->db->from('course');
        $this->db->order_by("id", "desc");
    ...
    public function filter_serial_learning_for_backend($category_id, $instructor_id, $price, $status, $search = '')
    {
    ...
    //video learning
    public function filter_webinar_for_backend($category_id, $instructor_id, $price, $status, $search = '')
    {
        $this->db->select('course.*,users.first_name,users.last_name');
        $this->db->from('course');
        $this->db->join('users', 'users.id=course.user_id');
    ...
    public function get_active_serial_learning($instructor_id = 0)
    {
        $this->db->where('status', 'active');
        $this->db->where('category_type', '2');
        if ($instructor_id > 0) {
    ...
    public function get_pending_serial_learning($instructor_id = 0)
    {
        if ($instructor_id > 0) {
            $this->db->where('user_id', $instructor_id);
        }
    ...
    public function get_free_serial_learning($instructor_id = 0)
    {
        if ($instructor_id > 0) {
            $this->db->where('user_id', $instructor_id);
        }
    ...
    public function get_paid_serial_learning($instructor_id = 0)
    {
        if ($instructor_id > 0) {
            $this->db->where('user_id', $instructor_id);
        }
    ...
    public function get_active_video_learning($instructor_id = 0)
    {
        $this->db->where('status', 'active');
    ...
    public function get_pending_video_learning($instructor_id = 0)
    {
        if ($instructor_id > 0) {
            $this->db->where('user_id', $instructor_id);
        }
    ...
    public function get_free_video_learning($instructor_id = 0)
    {
        if ($instructor_id > 0) {
            $this->db->where('user_id', $instructor_id);
        }
    ...
    public function get_paid_video_learning($instructor_id = 0)
    {
        if ($instructor_id > 0) {
            $this->db->where('user_id', $instructor_id);
        }
    ...
    public function get_active_webinar($instructor_id = 0)
    {
        $this->db->where('status', 'active');
        if ($instructor_id > 0) {
            $this->db->where('user_id', $instructor_id);
    ...
    public function get_pending_webinar($instructor_id = 0)
    {
        if ($instructor_id > 0) {
            $this->db->where('user_id', $instructor_id);
        }
    ...
    public function get_free_and_paid_courses($price_status = "", $instructor_id = "", $category_type = '0')
    {
        // echo $price_status;die;
        // if(!addon_status('scorm_course')){
        //     $this->db->where('course_type', 'general');
        // }
        $this->db->where('status', 'active');
        ...
        if (!isset($_GET['dashboard']) && (int)$category_type >= 0)
            $this->db->where('category_type', $category_type);
    ...
    public function add_quiz($course_id = "", $lesson_type = 'quiz')
    {
        $data['course_id'] = $course_id;
        $data['title'] = html_escape($this->input->post('title'));
        $data['section_id'] = html_escape($this->input->post('section_id'));
    ...
    public function get_courseAllCategory()
    {
        $this->db->select('*');
    ...
    //webinar
    //add video learning
    public function add_webinar($param1 = "")

    {
        ...
        $UploadArray['user_id'] = $this->session->userdata('user_id');
        ...
        {
            $UploadArray['is_free_course'] = 0;
        }
        
         if ($this->session->userdata('admin_login')) {
                 $UploadArray['status'] = 'active';
            } else {
                $UploadArray['status'] = 'pending';
            }

     
        $requirements = $this->trim_and_return_json($this->input->post('requirements'));
        $UploadArray['requirements'] = $requirements;
        ...
        $UploadArray['end_date'] = (!empty($_POST['end_date'])) ? date('Y-m-d',strtotime($_POST['end_date'])) : $UploadArray['proposed_date'];
        
        
        // echo '<pre>';print_r($UploadArray);die;
        ...
        if($_FILES['thumbnail']['name'])
            $UploadArray['thumbnail'] = $this->upload_file('uploads/thumbnails/course_thumbnails/', 'thumbnail',0);
            
        $this->db->insert('course', $UploadArray);
        $course_id = $this->db->insert_id();
        ...
        if ($_FILES["attachement"]['name'] != "") {
            move_uploaded_file($_FILES["attachement"]['tmp_name'], 'uploads/thumbnails/course_thumbnails/attachement_' . $course_media . '_' . get_frontend_settings('theme') . '_' . $course_id . '.jpg');
        }
        ...
        if ($course_id) {
            ...
            $fname = $get_user_data['first_name'];
            $title = $this->input->post('title');
            $p_date = $this->input->post('proposed_date');
            $p_time = $this->input->post('proposed_time');
            ...
            $numbers = array($mobile);
            $sender = urlencode(SMS_SENDERID);
            $message = rawurlencode("[MedEd]: Hello, $fname A webinar on $title has been scheduled on $p_date at $p_time. Checkout For more info: meded.testing@gmail.com.");
            $numbers = implode(',', $numbers);

            $this->session->set_flashdata('webinar_alert', "[MedEd]: Hello, $fname A webinar on $title has been scheduled on $p_date at $p_time. Checkout For more info: meded.testing@gmail.com.");
    ...
    //add video learning
    public function edit_webinar($param1 = "")

    {
        ...
        $course_id = $param1;
        $this->db->update('webinar', $data, 'id=' . $course_id);

        // Create folder if does not exist
        if (!file_exists('uploads/webinar/attachement/')) {
            mkdir('uploads/webinar/attachement/', 0777, true);
        }
        ...
        if ($_FILES["attachement"]['name'] != "") {
            //set preferences
            //file upload destination
            $upload_path = 'uploads/webinar/attachement/';
            $config['upload_path'] = $upload_path;
            //allowed file types. * means all types
            //allowed max file size. 0 means unlimited file size
            $config['max_size'] = '0';
            ...
            if ($is_file_error) {
                ...
                $data['attachement'] = $video_data['file_name'];
                $this->db->update('webinar', $data, 'id=' . $course_id);
            }
        }

        $this->session->set_flashdata('flash_message', get_phrase('webinar_has_been_updated_successfully'));
        //return $course_id;
        redirect(site_url('home/edit_webinar/' . $course_id), 'refresh');
    }

    // Get fill in blank questions
    public function get_blank_questions($quiz_id)
    {
        // $this->db->order_by("order", "asc");
        $this->db->where('quiz_id', $quiz_id);
        return $this->db->get('fill_in_blank');
    }
    // Add Quiz Questions
    public function add_blank_questions($quiz_id)
    {
        $question_type = $this->input->post('question_type');
        if ($question_type == 'fill_in_blank') {
            $response = $this->add_fill_in_blank_question($quiz_id);
            return $response;
        }
    }

    // multiple_choice_question crud functions
    function add_fill_in_blank_question($quiz_id)
    {
        if (sizeof($this->input->post('options')) != $this->input->post('number_of_options')) {
            return false;
        }
        ...
        $data['quiz_id']            = $quiz_id;
        $data['title']              = html_escape($this->input->post('title'));
        $data['number_of_blank']    = html_escape($this->input->post('number_of_options'));
        $data['type']               = 'fill_in_blank';
        $data['text']               = html_escape($this->input->post('text_area'));
        $data['answer']             = json_encode($this->input->post('options'));
        $this->db->insert('fill_in_blank', $data);
        return true;
    }
    public function delete_fill_in_blank($course_id, $quiz_id)
    {
        $this->db->where('id', $quiz_id);
        $this->db->delete('fill_in_blank');
        return true;
    }
    public function get_fill_in_blank_question_by_id($question_id)
    {
        $this->db->where('id', $question_id);
        return $this->db->get('fill_in_blank');
    }
    public function update_blank_questions($question_id)
    {
        $question_type = $this->input->post('question_type');
        if ($question_type == 'fill_in_blank') {
            $response = $this->update_fill_blank_question($question_id);
            return $response;
        }
    }
    // update multiple choice question
    public function update_fill_blank_question($question_id)
    {
        if (sizeof($this->input->post('options')) != $this->input->post('number_of_options')) {
            return false;
        }
        ...
        $this->db->where('id', $question_id);
        $this->db->update('fill_in_blank', $data);
        return true;
    }

    public function count_enrol_history_by_user_id($user_id = "")
    {
        return $this->db->get_where('enrol', array('user_id' => $user_id))->num_rows();
    }

    public function count_course_request_by_user_id($user_id = "")
    {
        return $this->db->get_where('tbl_course_request', array('user_id' => $user_id))->num_rows();
    }

    public function count_webinars_by_user_id($user_id = "")
    {
        return $this->db->get_where('watch_histories', array('student_id' => $user_id))->num_rows();
    }
    ...
    public function courses_enrol_history($user_id = 0)
    {
        ...
        if (!empty($_GET['is_free']) && $_GET['is_free'] != 'all') {
            $this->db->select('id as course_id');
            $this->db->from('course');
            $this->db->where('is_free_course', (strtolower($_GET['is_free']) == 'free') ? 1 : 0);
        ...
        // $this->db->where('course.status','active');
        if (!empty($CourseData))
            $this->db->where_in('enrol.course_id', array_column($CourseData, 'course_id'));
    ...
    public function count_enrol_history_by_course_id($course_id = "")
    {
        ...
        return $this->db->get_where('enrol', array('course_id' => $course_id))->num_rows();
    }
    ...
    public function get_course_revenue_by_id($id)
    {
        if (is_array($id)) {
    ...
    #---------------------------------------------------------------------------
    /*function for upload image with folder and return name*/
    public function upload_file($path = '', $file = '',$course_id = 0)
    {
        $file_name = '';
        if(!empty($path) && !empty($file))
        {
            $file_name = $path.$file.'_'.time().'_'.date('dmY').'_'.rand(111,999).$course_id.'.'.explode('.',$_FILES[$file]['name'])[1];
