...
// function for get login user role detail
if (! function_exists('getrolecode'))
{
    function getrolecode($user_id = 0)
    {
        $CI =&  get_instance();
        $CI->load->database();
        ...
        $UserData = $CI->db->select('admin_role')->where('id',$user_id)->get('users')->row_array();
        
        return (empty($UserData)) ? '5' : $UserData['admin_role'];
    }
}
// ------------------------------------------------------------------------
