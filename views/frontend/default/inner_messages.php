...
<?php
    ...
    $messages = $this->db->order_by('message_id','ASC')->get_where('message', array('message_thread_code' => $message_thread_code))->result_array();?>
    <div class="message-details d-show">
                ...
                <?php if ($message['sender'] == $this->session->userdata('user_id')): ?>
                    <div class="message-box-wrap me">
                        <div class="message-box">
                            <div class="time"><?php echo date('D, d-M-Y H:i', $message['timestamp']); ?></div>
        ...
        <div class="message-footer">
            <form class="" action="<?php echo site_url('home/my_messages/send_reply/'.$message_thread_code); ?>" method="post">
                <textarea class="form-control message-input" name="message" placeholder="<?php echo site_phrase('type_your_message'); ?>..."></textarea>
                <button class="btn send-btn send-message-btn" type="button"><?php echo site_phrase('send'); ?></button>
            </form>
        </div>
    </div>
<?php endif; ?>
