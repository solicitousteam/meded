...
class Home extends CI_Controller
{
    ...
    {
        $this->email_model->register('paritosh.golash@gmail.com', 'Paritosh', 'Golash');
    }
    public function verification_code($role = 1)
    {
        if ($role == 2) {
    ...
    public function demo_video($id = 0)
    {
        ...
        $page_data['page_title'] = "Demo Video";
    ...
    public function success($id = 0)
    {
        ...
        $page_data['page_title'] = "Received Request";

        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function subscription_plans($id = 0)
    {
        if ($this->session->userdata('user_id') == null) {
            redirect(site_url('home/login'), 'refresh');
        }
        // $page_data['user_id'] = $id;
        $page_data['page_name'] = "subscription_plans";
        $page_data['page_title'] = "Subscription Plans";

        $data = $this->db->select('first_name,last_name')->where('id', $id)->get('users')->row_array();
        $page_data['user_name'] = (!empty($data)) ? $data['first_name'] . ' ' . $data['last_name'] : '';
    ...
    public function my_messages($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == 'read_message') {
            ...
            if (!empty($_GET['newmessage'])) {
                redirect(base_url('home/my_messages/read_message/' . $message_thread_code));
            }

            $messages = $this->db->order_by('message_id','ASC')->get_where('message', array('message_thread_code' => $_POST['message_thread_code']))->result_array();

            $html = '';
            foreach ($messages as $message) {
                if ($message['sender'] == $this->session->userdata('user_id')) {
                    $html .= '<div class="message-box-wrap me">
                                <div class="message-box">
                                    <div class="time">' . date('D, d-M-Y H:i', $message['timestamp']) . '</div>
                                    <div class="message">' . $message['message'] . '</div>
                                </div>
                            </div>';
                } else {
                    $html .= '<div class="message-box-wrap">
                        <div class="message-box">
                            <div class="time">' . date('D, d-M-Y H:i', $message['timestamp']) . '</div>
                            <div class="message">' . $message['message'] . '</div>
                        </div>
    ...
    public function support_contact($param1 = "", $param2 = "")
    {
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('home'), 'refresh');
        }

        $page_data['page_name'] = "support_contact";
        $page_data['page_title'] = site_phrase('support_contact');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function subscriptionHistory()
    {
        ...
        $page_data['page_name']  = "subscriptionHistory";
    ...
    public function profile($param1 = "")
    {
        ...
        $page_data['user_profile_details'] = $this->db->where('id', $this->session->userdata('user_id'))->get('users')->row_array();
        // print_r($page_data['user_profile_details']);die;
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function update_profile($param1 = "")
    {
        if ($param1 == 'update_basics') {
            ...
            if (isset($_FILES['user_image']) && $_FILES['user_image']['name'] != "") {
                unlink('uploads/user_image/' . $this->db->get_where('users', array('id' => $this->session->userdata('user_id')))->row('image'));

                $data['last_modified'] = strtotime(date("Y-m-d H:i:s"));

                $UserDetail = $this->db->where('id', $this->session->userdata('user_id'))->get('users')->row();
    ...
    {
        $page_data['page_name'] = 'sign_up_as';
    ...
    public function student_sign_up()
    {
        ...
        $page_data['page_name'] = 'student_sign_up';
        $page_data['page_title'] = site_phrase('sign_up');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    function save_banner_request()
    {
        ...
        $last_insert_id = $this->db->insert('banner_request', $data);
        ...
        redirect(site_url('home/banner_request'), 'refresh');
    }

    function submit_support_contact()
    {
        ...
        $InsertArray = [
            ...
            'email' => $_POST['email'],
            'phone' => trim($_POST['area_code']) . trim($_POST['phone_number']),
            'how_we_contacted' => $_POST['how_we_contacted'],
            'problem_with' => $_POST['problem_with'],
            'description' => $_POST['description'],
        ];

        $last_insert_id = $this->db->insert('support_contact', $InsertArray);

        if ($last_insert_id) {
            $this->session->set_flashdata('flash_message', get_phrase('query_submitted_successfully'));
        } else {
            $this->session->set_flashdata('error_message', get_phrase('please_enter_your_query'));
        }
        redirect(site_url('home/support_contact'), 'refresh');
    }

    function serial_learning()
    {

        $page_data['page_name'] = "serial_learning";

        $page_data['page_title'] = "Serial Learning";
        $page_data['webinar'] = $this->db->get('serial_learning')->result();
    ...
    function add_serial()
    {
        ...
        $page_data['page_title'] = "Add Serial";
        $page_data['serial'] = $this->db->get('serial_learning')->result();
        $this->load->view('backend/index', $page_data);
    }
    ...
    function edit_serial()
    {
        ...
        $page_data['serial'] = $this->db->where('id', $id)->get('serial_learning')->result();

        $page_data['page_name'] = "edit_serial";
        $page_data['page_title'] = "Edit Serial";
        $page_data['webinar'] = $this->db->get('serial_learning')->result();
        $this->load->view('backend/index', $page_data);
    }
    ...
    function webinar()
    {
        ...
        $page_data['page_title'] = "Webinar";
        $page_data['webinar'] = $this->db->where('created_by', $loggedInUserId)->get('webinar')->result();
        $this->load->view('backend/index', $page_data);
    }
    ...
    function add_webinar()
    {
        ...
        $page_data['page_title'] = "Add Webinar";
        $page_data['webinar'] = $this->db->get('webinar')->result();
        $this->load->view('backend/index', $page_data);
    }
    ...
    function edit_webinar($id)
    {
        ...
        $page_data['webinar'] = $this->db->where('id', $id)->get('webinar')->row();

        $page_data['page_name'] = "edit_webinar";
        $page_data['page_title'] = "Edit Webinar";
        // $page_data['webinar'] = $this->db->get('webinar')->result();
        $this->load->view('backend/index', $page_data);
    }
    
    /* 26-05-2022 Start */
    #function for get the curriculum
    public function getcurriculumnpreview()
    {
        $html ='';
        $status = 400;
        ...
        if(!empty($_POST['curriculum']))
        {
            ...
            '<div class="course-curriculum-box">
                <div class="course-curriculum-title clearfix">
                    ...
                    <div class="float-right">
                        <span class="total-lectures d-none">
                            5 Lessons </span>
            ...
            foreach($curriculum as $key => $value)
            {
                ...
                '   <div class="lecture-group-wrapper">
                        <div class="lecture-group-title clearfix" data-toggle="collapse" data-target="#curriculum-'.$key.'" aria-expanded="'.(($key == 0) ? 'true' : 'false').'">
                            <div class="title float-left">
                                '.ucwords($value['name']).'
                            </div>
                        </div>
                        <div id="curriculum-'.$key.'" class="lecture-list collapse '.(($key == 0) ? 'show' : '').'">
                            <ul>';
                        foreach($value['subdata'] as $subvalue)
                        {
                            $html .=
                            '<li class="lecture has-preview">
                                <span class="lecture-title">'.ucwords($subvalue['name']).'</span>
                                <span class="lecture-time float-right">'.ucwords($subvalue['type']).'</span>
        ...
        echo json_encode(['Status'=>$status,'Data'=>$html]);die;
    }
    /* 26-05-2022 Close */
}